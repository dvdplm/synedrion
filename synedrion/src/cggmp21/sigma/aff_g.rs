...
impl<P: SchemeParams> AffGProof<P> {
    pub fn new(
        ...
        let cap_e = setup.commit_secret_wide(&alpha, &gamma).to_wire();
        let cap_s = setup.commit_secret_wide(secret.x, &m).to_wire();
        let cap_f = setup.commit_secret_wide(&beta, &delta).to_wire();

        // NOTE: deviation from the paper to support a different $D$
        // (see the comment in `AffGPublicInputs`)
        // Original: $s^y$. Modified: $s^{-y}$
        let cap_t = setup.commit_secret_wide(&(-secret.y), &mu).to_wire();
    ...
    pub fn verify(&self, public: AffGPublicInputs<'_, P>, setup: &RPParams<P::Paillier>, aux: &impl Hashable) -> bool {
        ...
        if setup.commit_pub_wide(&self.z1, &self.z3) != &cap_e * &cap_s.pow(&e) {
            return false;
        }
        ...
        if setup.commit_pub_wide(&self.z2, &self.z4) != &cap_f * &cap_t.pow(&e) {
            return false;
        }
