...
impl<P: SchemeParams> LogStarProof<P> {
    pub fn new(
        ...
        let cap_s = setup.commit_secret_wide(secret.x, &mu).to_wire();
        ...
        let cap_d = setup.commit_secret_wide(&alpha, &gamma).to_wire();
    ...
    pub fn verify(
        ...
        if setup.commit_pub_wide(&self.z1, &self.z3) != &cap_d * &cap_s.pow(&e) {
            return false;
        }
